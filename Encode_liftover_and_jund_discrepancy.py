...
peak_bed_list = glob.glob("/gpfs/gpfs1/home/schhetri/for_chris/batch_I/jun_d_test/idr_passed_peaks_total/lifted_bedfiles/*.bed")
...
liftover_temp_dir = join(liftover_output_dir, "temp")
if not os.path.exists(liftover_temp_dir):
    os.makedirs(liftover_temp_dir)

convert_filelist = glob.glob("/gpfs/gpfs1/home/schhetri/for_chris/batch_I/jun_d_test/idr_passed_peaks_total/*")
...
for each_file in convert_filelist:
    ...
    df.to_csv(join(liftover_temp_dir, file_name), header=False, index=False, sep="\t")

    input_file = join(liftover_temp_dir, file_name)
    ...
    os.environ["unmapped_file"] = splitext(basename(each_file))[0] + "_hg19_unmapped" 
    os.environ["output_dir"] = liftover_output_dir
    os.environ["temp_dir"] = liftover_temp_dir


    os.system("$Liftover_path/liftOver $input_file $chain_file $output_dir/$output_file $temp_dir/$unmapped_file")
...
cpg_bed_edited = splitext(basename(cpg_bed_file))[0] + "_chrM_edited" + splitext(cpg_bed_file)[1]
...
cpg_df = pd.read_csv(Cpg_bed_file.fn, header=None, sep="\t")
cpg_df.columns = ["chrom", "start", "end", "meth", "unmeth", "strand"]
cpg_df["coverage"] =   cpg_df["meth"] + cpg_df["unmeth"]
#Cpg_bed_file_df = cpg_df[~(cpg_df["coverage"] > 200)]
Cpg_bed_file_df["meth_perc"] = cpg_df["meth"] / (cpg_df["meth"] + cpg_df["unmeth"]) * 100

cpg_file_with_cov_more_than_200 ="/gpfs/gpfs1/home/schhetri/for_chris/batch_I/jun_d_test/idr_passed_peaks_total/lifted_bedfiles"
Cpg_bed_file_df.to_csv(join(cpg_file_with_cov_more_than_200, "Cpg_file_with_more_than_200_coverage" ), index=False, header=False, sep="\t")
Cpg_bed_file = pybedtools.BedTool.from_dataframe(Cpg_bed_file_df)
...
master_tf_df.to_csv(join(output_dir, "final_JUND_average_methylation_comparison_without_chrM.bed"), sep ="\t", header = True, index = False)
print master_tf_df
